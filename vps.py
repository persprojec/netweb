...
async def send_db_to_owner():
    ...
    with open(DB_FILE, "rb") as f:
        data_bytes = f.read()
    bio_stream = io.BytesIO(data_bytes)
    bio_stream.name = "db.json"
    bio_stream.seek(0)
    await telegram_app.bot.send_document(
        chat_id=OWNER_CHAT_ID,
        document=InputFile(bio_stream, filename="db.json"),
...
def save_db_and_notify(data):
    ...
    if telegram_app:
        # one-off asyncio.run to send update
        asyncio.run(send_db_to_owner())
...
def decrement_token_days():
    try:
        ...
        err = str(e)
        if telegram_app:
            bio_stream = io.BytesIO(err.encode("utf-8"))
            bio_stream.name = "error.txt"
            bio_stream.seek(0)
            asyncio.run(
                telegram_app.bot.send_document(
                    chat_id=OWNER_CHAT_ID,
                    document=InputFile(bio_stream, filename="error.txt"),
                    caption="❌ Error",
                )
            )
...
async def send_error_to_owner(txt):
    if telegram_app:
        bio_stream = io.BytesIO(txt.encode("utf-8"))
        bio_stream.name = "error.txt"
        bio_stream.seek(0)
        try:
            await telegram_app.bot.send_document(
...
async def users_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        bio_stream = io.BytesIO(jb)
        bio_stream.name = "users.json"
        bio_stream.seek(0)
        await update.message.reply_document(
            document=InputFile(bio_stream, filename="users.json"),
